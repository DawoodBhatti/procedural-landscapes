...
# --- Zoom settings ---
const MIN_ZOOM := 1.035        # smaller = zoom out further
const MAX_ZOOM := 7.0         # larger = zoom in further
...
const START_ZOOM: Vector2 = Vector2(4.038061, 4.038061)

# --- Pan settings ---
#const PAN_SPEED := 900.0        # pixels per second at zoom = 1


var target_zoom: Vector2 = START_ZOOM
var dragging := false
...
func _ready() -> void:
	...
	#adjust camera when distribution size gets set
	signals.connect("dist_area_selected", _on_dist_area_selected)
...
func _on_dist_area_selected (size: String) -> void:
	if size == "Small":
		_set_zoom(Vector2(4.038061, 4.038061))
	elif size == "Medium":
		_set_zoom(Vector2(2.65509, 2.65509))
	elif size == "Large":
		_set_zoom(Vector2(1.977137, 1.977137))
